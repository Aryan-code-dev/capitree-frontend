...
function Flow() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:3001/load/initialData?owner=1');
  ...
  const handleTabClose = async () => {
    ...
    const edgesData = localStorage.getItem('edges');
      ...
      try {
        const response = await axios.post('http://localhost:3001/update/localUpdate', data);
